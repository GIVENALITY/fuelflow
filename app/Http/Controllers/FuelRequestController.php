...
class FuelRequestController extends Controller
{
    ...
    public function edit(FuelRequest $fuelRequest)
    {
        ...
        if ($fuelRequest->status !== FuelRequest::STATUS_PENDING || 
    ...
    public function assign(FuelRequest $fuelRequest)
    {
        $user = Auth::user();
        ...
        $fuelRequest->update([
            'assigned_pumper_id' => $user->id,
            'status' => FuelRequest::STATUS_IN_PROGRESS
        ]);
        
        return redirect()->route('fuel-requests.index')
            ->with('success', 'Fuel request assigned successfully!');
    }
    ...
    public function dispense(Request $request, FuelRequest $fuelRequest)
    {
        ...
        $fuelRequest->dispense($user->id, $validated['quantity_dispensed'], $validated['notes']);
